...
enable_previewd() {
  ...
  if ! systemctl --user enable --now "$PREVIEW_SERVICE"; then
    warn "$PREVIEW_SERVICE could not be started. The applet will remain in Safe Core fallback until previewd is available."
    return
  fi
  ...
  if systemctl --user is-active --quiet "$PREVIEW_SERVICE"; then
    ...
    warn "$PREVIEW_SERVICE is not active. The applet will use Safe Core fallback."
  fi
}
...
main() {
  ...
  printf 'Safe Core remains the fallback if tihulu-previewd is unavailable or degraded.\n'
}
